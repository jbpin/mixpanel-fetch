...
var create_client = function(token, config) {
    ...
    */
    metrics.send_request = function(endpoint, data, callback) {
        ...
        var url = metrics.config.host + endpoint;
        var req = Object.keys(request_data).reduce(function(acc, key){
            acc.push(encodeURIComponent(key) + '=' + encodeURIComponent(request_data[key]));
            return acc;
        }, []);
        ...
        .then( function(response){
            ...
            var e;
            if (metrics.config.verbose && result.status != 1) {
                e = new Error("Mixpanel Server Error: " + result.error);
            }
            if(!result){
                e = new Error("Mixpanel Server Error: " + result);
            }
            callback(e);
        })
        .catch( function(e) {
